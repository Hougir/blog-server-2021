...
public class Userservice {
    ...
    public R login(AuthBo bo) {
        ...
        log.info("数据库校验");
        //redis获取短信验证码验证
        String content = (String)cacheComponent.get(CacheKey.LOAN_USER_SMS_CODE_LOGIN.getKey(bo.getPhone()));
        ...
        cacheComponent.add(CacheKey.LOAN_USER_LOGIN_TOKEN.getKey(user.getId().toString()),token, CommConst.EXPIRED_TIME);
        return R.ok(token);
    }

    public R logout(String token) {
        log.info("logout: {}",token);
        TUser user = (TUser) cacheComponent.get(CacheKey.LOAN_USER_LOGIN_TOKEN_USER.getKey(token));
        if (null != user && null != user.getId()){
            //删除redis缓存token值
            boolean b = cacheComponent.remove(CacheKey.LOAN_USER_LOGIN_TOKEN.getKey(user.getId().toString()));
            if (!b) return R.error().message(ResultMsg.FAILED_TO_DELETE_TOKEN);
        }
        //删除用户
        Boolean b = cacheComponent.remove(CacheKey.LOAN_USER_LOGIN_TOKEN_USER.getKey(token));
    ...
    public R sendSms(String phone) {
        ...
        String content = (String)cacheComponent.get(CacheKey.LOAN_USER_SMS_CODE_LOGIN.getKey(phone));
